    ...
    function displayStaffData(data) {
        const tableBody = document.getElementById("corps-actualite"); // Correction de la méthode pour obtenir l'élément
            ...
            data.forEach(item => {
                // Parcourir toutes les personnes de l'item
                    ...
                    newRowForm.querySelector('form').addEventListener('submit', event => {
                        ...
                        updateStaffData(`/Updateactualite/${item._id}`, new FormData(event.target));
                    });
            });
    }
